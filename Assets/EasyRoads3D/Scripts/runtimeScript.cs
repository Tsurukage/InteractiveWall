...
public class runtimeScript : MonoBehaviour {
	...
	void CreateRoadNetwork()
    {
		...
		var road1 = roadNetwork.GetRoadByName("Default Road 001");

		// Get a reference to connection object "Default X Crossing 0001" in the scene
		var conn1 = roadNetwork.GetConnectionByName("Default X Crossing 001");

		// First find the nearest connection index to the start of road1 by passing the first marker index position
		var index = conn1.FindNearestConnectionIndex(road1.GetMarkerPosition(0));
		...
		var connPrefab = roadNetwork.GetConnectionPrefabByName("Default X Crossing");

		// Create a new instance of this prefab and align and connect it to the end of road object 1
		var conn2 = road1.AttachToEnd(connPrefab);
		...
		var roadType = roadNetwork.GetRoadTypeByName("Default Road");

		// Set the road marker positions for the new road
		var roadMarkers = new Vector3[2] {new Vector3(410f, 0f, 555f), new Vector3(465f, 0f, 555f)};

		// Create the road object
		var road2 = roadNetwork.CreateRoad("Default Road 002", roadType, roadMarkers);
	...
	void Update () {
		...
		if(road != null){
			...
			var rSpeed = (deltaT * speed);
		
			distance += rSpeed;

			// pass the current distance to get the position on the road
//			Debug.Log(road);
			var v = road.GetPosition(distance, ref currentElement);
			v.y += 1;
